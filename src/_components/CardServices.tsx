...
import { CalendarIcon, StarIcon, ChevronLeft, ChevronRight } from "lucide-react";
...
const CardServices = ({
  ...
  useEffect(() => {
    ...
    while (slot < end) {
      const slotEnd = new Date(
        slot.getTime() + (selectedServiceOption?.duration || BarberShopService.duration) * 60000,
      );
  ...
  const handleCreateBooking = useCallback(async () => {
    if (!selectBarber || !selectedDate || !selectedTime) {
    ...
    const successData = {
      serviceName: BarberShopService.name,
      ...
      time: selectedTime,
      price: serviceOption ? Number(serviceOption.price) : getLowestPrice
    };
  ...
  }, [selectBarber, selectedDate, selectedTime, BarberShopService.id, selectedServiceOption, barbers, createBookingWithSuccess, getLowestPrice]);
            ...
            <Sheet>
              ...
              >
                ...
                <div className="scrollbar-sheet flex-1 overflow-y-auto">
                  ...
                  {/* Step 2: Escolher Opção do Serviço */}
                  {currentStep === 2 && (
                    <div className="p-5 space-y-4">
                      <div className="text-center">
                        <h3 className="text-lg font-semibold mb-2">Escolha a Opção</h3>
                        <p className="text-sm text-muted-foreground">Selecione a opção específica do serviço {BarberShopService.name}</p>
                        <p className="text-xs text-red-400 mt-1">* Seleção obrigatória</p>
                      </div>
                      
                      <div className="grid gap-3">
                        {getServiceOptions(BarberShopService).map((option) => (
                          <div
                            key={option.id}
                            className={`p-3 rounded-lg border-2 cursor-pointer transition-all duration-200 ${
                              selectedServiceOption?.id === option.id
                                ? "border-primary bg-primary/5 shadow-lg"
                                : "border-border hover:border-primary/50 hover:bg-accent-hover"
                            }`}
                    ...
                    <div className="space-y-4">
                      ...
                      {selectedDate && (
                        <div className="px-5 pb-5">
                          <h3 className="py-2 text-sm font-semibold mb-3">Escolha o Horário</h3>
                          <div className="flex gap-2 overflow-auto [&::-webkit-scrollbar]:hidden">
                            ...
                            {availableTimes.map((time) => (
                              <Button
                                ...
                                className="rounded-2xl border-solid font-normal text-card-foreground whitespace-nowrap"
                          ...
                          <Card className="px-0 py-3">
                            <CardContent className="space-y-2 px-2">
                              <div className="flex justify-between px-2">
                                ...
                                <p className="text-sm font-bold">
                                  {Intl.NumberFormat("pt-BR", {
                                    style: "currency",
                                    currency: "BRL",
                                  }).format(selectedServiceOption?.price || Number(BarberShopService.price))}
                                </p>
                              </div>
                              <div className="flex justify-between px-2">
                              ...
                              <div className="flex justify-between px-2">
                                ...
                                <span className="text-sm">
                                  {selectedServiceOption?.duration || BarberShopService.duration} Min
                                </span>
                              </div>
                              <div className="flex justify-between px-2">
